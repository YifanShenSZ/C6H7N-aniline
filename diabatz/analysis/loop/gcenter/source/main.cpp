...
argparse::ArgumentParser parse_args(const size_t & argc, const char ** & argv) {
    ...
    parser.add_argument("--NNH+"      , 1, true, "number of grids along N-H +, default = 100");
    parser.add_argument("--NNH-"      , 1, true, "number of grids along N-H -, default = 100");
...
int main(size_t argc, const char ** argv) {
    ...
    size_t NNHplus  = 100;
    if (args.gotArgument("NNH+" )) NNHplus  = args.retrieve<size_t>("NNH+");
    size_t NNHminus = 100;
    if (args.gotArgument("NNH-" )) NNHminus = args.retrieve<size_t>("NNH-");
    ...
    std::cout << "shift NH by " << shift_NH / 1.8897261339212517 << " A, CNH2 by " << shift_CNH2 << '\n'
              << "NNH+ = " << NNHplus << ", NNH- = " << NNHminus << ", dNNH = " << dNH / 1.8897261339212517 << " A\n"
              << "NCNH2 = " << NCNH2 << ", dCNH2 = " << dCNH2 << '\n' << std::endl;

    at::Tensor integral = integrate_loop(HdKernel, shift_NH, shift_CNH2, NNHplus, NNHminus, dNH, NCNH2, dCNH2);
    std::cout << "integral =\n";
